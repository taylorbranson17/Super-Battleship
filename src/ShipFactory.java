...
public abstract class ShipFactory {
    ...
    protected abstract Direction getDirection(List<Direction> directions, String shipName); // Gets/Generates a
                                                                                            // direction
    ...
    public void placeAShip(Ship ship) {
        Coord leadCoord;
        List<Direction> validDirects;
        while (true) {
            leadCoord = getLeadCoord(ship.getName());
            validDirects = filterOverlap(leadCoord, ship.getLength());
        ...
        Direction direction = getDirection(validDirects, ship.getName()); // Will be random in Automatic and
                                                                             // user-entered in Manual
    ...
    // Returns a list of valid directions given the ship's leading coordinate
    // (leadCoord) and its length
    ...
    protected ArrayList<Coord> genShipCoords(Coord leadCoord, int length, Direction direction) {
        ...
        // Iterate through the length of the ship and add a new coordinate for each
        // space
    ...
    public List<Direction> filterOverlap(Coord leadCoord, int length) {
        ...
        // if totalShipCoords contains any of them, it removes the directions from valid
        // list.
